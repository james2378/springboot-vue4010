<template>
	<div class="diy_details_box diy_div_department_management">
		<div class="warp">
			<div class="container">
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					...
					source_field: "department_management_id",
					source_id: this.obj["department_management_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/department_management/set?department_management_id=' + _this.obj["department_management_id"], {
								praise_len
							}, (res) => {
								if(res.result){
