<template>
	<div class="diy_details_box diy_div_candidate_user">
		<div class="warp">
			<div class="container">
				...
				<div class="row">
						...
						<!-- 图片 -->
							...
							<div class="details_btn_wrap">
								<button class="btn btn-primary details_btn" v-if="$check_action('/candidate_user/details','set')" @click="change_collect()" >收藏</button>
							</div>
						<!-- 富文本 -->
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					...
					source_field: "candidate_user_id",
					source_id: this.obj["candidate_user_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/candidate_user/set?candidate_user_id=' + _this.obj["candidate_user_id"], {
								praise_len
							}, (res) => {
								if(res.result){
