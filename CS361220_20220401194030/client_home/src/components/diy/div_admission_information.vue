<template>
	<div class="diy_details_box diy_div_admission_information">
		<div class="warp">
			<div class="container">
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					...
					source_field: "admission_information_id",
					source_id: this.obj["admission_information_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/admission_information/set?admission_information_id=' + _this.obj["admission_information_id"], {
								praise_len
							}, (res) => {
								if(res.result){
