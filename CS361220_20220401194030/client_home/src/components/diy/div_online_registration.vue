<template>
	<div class="diy_details_box diy_div_online_registration">
		<div class="warp">
			<div class="container">
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					...
					source_field: "online_registration_id",
					source_id: this.obj["online_registration_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/online_registration/set?online_registration_id=' + _this.obj["online_registration_id"], {
								praise_len
							}, (res) => {
								if(res.result){
