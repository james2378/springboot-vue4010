<template>
	<div class="diy_details page_registration_process" id="registration_process_details">
		<div class='warp'>
			<div class='container'>
				<div class='row'>
					<div class='col'>
						<div class='card_registration_process'>
							<div_registration_process :obj="obj"></div_registration_process>
						</div>
					</div>
				</div>
			</div>
...
<script>
	import div_registration_process from "@/components/diy/div_registration_process.vue";
	import mixin from "@/mixins/page.js";
	export default {
		mixins:[mixin],
		components:{
			div_registration_process
		},
		data(){
			return{
		...
		methods:{
			...
			get_collect(obj) {
				...
				var query = {
					...
					source_field: "registration_process_id",
					source_id: obj["registration_process_id"],
					user_id
				};
		 ...
		 /****
			...
			change_collect() {
				...
				var query = {
					source_table: "registration_process",
					source_field: "registration_process_id",
					source_id: this.obj["registration_process_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/registration_process/set?registration_process_id=' + _this.obj["registration_process_id"], {
								praise_len
							}, (res) => {
								if(res.result){
