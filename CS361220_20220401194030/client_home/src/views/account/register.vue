						...
						<div class="card">
							...
							<div class="form_register">
								<b-form>
									...
									<!--考生用户-->
									<form_candidate_user v-if="form.user_group === '考生用户'" :form="form_sub"></form_candidate_user>
...
<script>
	...
	/* 考生用户 */
	import form_candidate_user from "@/components/diy/form_candidate_user";
				...
				if(!ret && user_group){
					if(user_group === "考生用户"){
						...
						if(!ret && !form_sub["student_number"]){
							...
							res = await this.$get("~/api/candidate_user/count?", {"student_number": form_sub["student_number"]});
							if(res.result){
								ret = "学号已存在!";
							}
						}
